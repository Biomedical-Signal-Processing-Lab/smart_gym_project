# advice.py
from __future__ import annotations
from typing import Dict, List, Optional
import random

# ──────────────────────────────────────────────────────────────
# 점수 → 버킷
# ──────────────────────────────────────────────────────────────
def score_bucket(score: Optional[int]) -> str:
    s = 0 if score is None else int(score)
    if s <= 50:   return "danger"     # 0~50
    if s <= 70:   return "improve"    # 51~70
    if s <= 89:   return "good"       # 71~89
    return "perfect"                  # 90~100


# ──────────────────────────────────────────────────────────────
# 기본 멘트 (운동별/버킷별)
# 필요 시 자유롭게 수정/확장
# ──────────────────────────────────────────────────────────────
BASE: Dict[str, Dict[str, List[str]]] = {
    "squat": {
        "danger": [
           # "무릎이 너무 앞으로 나가요! 발끝을 넘지 않게.",
            "엉덩이를 더 뒤로 빼고, 중심을 발뒤꿈치로 옮겨보세요."
           # "허리가 말리고 있어요. 가슴을 들고 중립척추 유지!"
        ],
        "improve": [
           # "자세가 불안정해요. 속도를 줄이고 중심을 잡아보세요.",
           # "무릎 방향을 바깥으로! 발과 무릎 라인 맞추기.",
            "조금만 더 깊이 앉고 코어 긴장 유지."
        ],
        "good": [
            #"거의 완벽해요! 무릎 흔들림만 조금 줄여보세요.",
            "힙 각도 좋아요."
        ],
        "perfect": [
            #"완벽! 무릎·힙 각도 이상적이에요",
            "균형과 깊이 모두 훌륭합니다."
        ],
    },

    "leg_raise": {
        "danger": [
            "허리가 뜹니다! 허리가 바닥에서 떨어지지 않게 복부에 힘.",
            "다리를 급하게 내리면 허리에 무리. 천천히 통제하세요.",
            "복부 힘이 풀렸어요. 배꼽을 바닥으로 누르듯 고정!"
        ],
        "improve": [
            "허리가 살짝 들려요. 코어로 허리 눌러 고정!",
            "다리 각도를 일정하게—속도 제어에 집중.",
            "고관절에만 힘이 몰려요. 복부로 들어올리세요."
        ],
        "good": [
            "좋아요! 코어 긴장 잘 유지 중. 하강 속도만 일정하게.",
            "허리 안정성 좋아요. 마지막 구간 집중!",
            "자세 깔끔합니다. 0.5초 정지로 컨트롤 업."
        ],
        "perfect": [
            "완벽! 복부 긴장·다리 각도·허리 고정 모두 훌륭해요.",
            "코어 컨트롤이 인상적입니다.",
            "프로 수준의 레그 레이즈예요!"
        ],
    },

    "shoulder_press": {
        "danger": [
            "팔꿈치가 너무 아래로 떨어져요. 어깨 라인 아래로 내려가지 않게!",
            "허리가 젖혀집니다. 코어 조이고 중립 유지.",
            "팔이 비대칭이에요. 양쪽 팔꿈치를 동일 궤도로 올리세요."
        ],
        "improve": [
            "팔꿈치 각도가 불안정해요. 손목-팔꿈치-어깨 일직선 정렬.",
            "내릴 때 속도를 낮추고 반동을 줄여보세요.",
            "팔꿈치가 벌어져요. 살짝 안쪽으로."
        ],
        "good": [
            "안정적! 끝 지점에서 0.5초 멈춤으로 컨트롤 업.",
            "팔꿈치 궤적 거의 일정. 살짝만 더 안쪽으로.",
            "어깨 안정성 훌륭. 완전 이완은 피하세요."
        ],
        "perfect": [
            "완벽한 궤적과 대칭! 코어·어깨 안정 훌륭",
            "프로 수준, 흔들림 없이 정확한 라인.",
            "밸런스와 힘 전달이 이상적입니다."
        ],
    },

    "lateral_raise": {
        "danger": [
            "팔을 너무 높이 들어요. 어깨에 무리!",
            "반동이 커요. 상체 흔들림 없이 올리기.",
            "손목이 꺾여요. 팔 전체를 일직선으로."
        ],
        "improve": [
            "양쪽 팔 높이를 맞추세요. 대칭 유지.",
            "팔꿈치 약간 구부리고 어깨로 들어올리기.",
            "어깨보다 손이 먼저 올라가요. 팔꿈치를 리드로."
        ],
        "good": [
            "좋아요! 각도 안정적. 속도 일정하게 유지.",
            "대칭이 잘 맞아요. 마무리 구간 0.5초 멈춤.",
            "어깨 중심으로 들고 있어요. 아주 깔끔해요."
        ],
        "perfect": [
            "완벽! 각도·궤적·대칭 모두 이상적",
            "반동 없이 어깨 힘으로 완전 제어.",
            "프로 수준의 레터럴 레이즈입니다."
        ],
    },

    "pushup": {
        "danger": [
            "팔꿈치가 너무 벌어져요. 몸통 가까이로!",
            "허리가 꺾입니다. 코어 조이고 몸 일직선.",
            "무릎이 흔들려요. 라인부터 안정화하세요."
        ],
        "improve": [
            "팔꿈치 각도 일정하게—천천히 내려가며 제어.",
            "무릎 흔들림 줄이고 어깨-엉덩이-무릎 라인 유지.",
            "손목 아래에 팔꿈치가 오도록 정렬하세요."
        ],
        "good": [
            "좋아요! 상체 움직임 자연스러움. 팔 대칭만 조금 더.",
            "코어 안정 훌륭. 마지막 구간 복부 긴장 유지!",
            "속도 일정, 범위 정확. 아주 좋습니다."
        ],
        "perfect": [
            "완벽한 푸시업! 각도·속도·대칭 이상적",
            "몸 전체가 일직선, 컨트롤 최고.",
            "프로 수준의 수행이에요."
        ],
    },

    "burpee": {
        "danger": ["자세가 무너져요. 속도를 낮추고 단계별로 정확히!"],
        "improve": ["반동 대신 근육으로. 착지·복귀 리듬 일정하게."],
        "good": ["좋아요! 리듬 유지하고 코어 조금 더 조이기."],
        "perfect": ["완벽! 리듬·정확도 모두 훌륭"],
    },

    "jumping_jack": {
        "danger": ["팔/다리 범위가 불안정. 동작 폭을 일정하게!"],
        "improve": ["팔·다리 타이밍 동기화, 반동 줄이기."],
        "good": ["좋아요! 속도 일정, 범위 유지."],
        "perfect": ["완벽! 타이밍과 대칭이 훌륭"],
    },

    "generic": {
        "danger": ["위험합니다. 속도를 낮추고 정렬을 먼저 교정하세요."],
        "improve": ["개선 중! 범위와 속도를 일정하게 유지해보세요."],
        "good": ["좋습니다. 세부 정렬만 다듬으면 완벽해요."],
        "perfect": ["완벽! 현재 패턴을 유지하세요"],
    },
}


# ──────────────────────────────────────────────────────────────
# 상황별 보정 팁(선택적으로 덧붙임)
# ctx={"elbow_not_deep": True, ...}
# ──────────────────────────────────────────────────────────────
TIP_MAP: Dict[str, str] = {
    "elbow_not_deep":   "팔꿈치를 더 접어 깊이를 확보하세요.",
    "elbow_flare":      "팔꿈치가 벌어집니다. 몸통 가까이로.",
    "knee_more_extend": "무릎을 더 펴 안정적인 라인을 만들어요.",
    "knee_valgus":      "무릎이 안쪽으로 말립니다. 바깥 향하게.",
    "hip_back":         "엉덩이를 더 뒤로 빼서 중심을 잡아보세요.",
    "back_arch":        "허리 과신전 주의! 코어 조이고 중립 유지.",
    "tilt_instability": "상체 흔들림이 커요. 속도를 낮추고 반동 금지.",
}


# ──────────────────────────────────────────────────────────────
# 즉시 반복 방지 선택기: 직전 선택은 다음 호출에서 제외
# 키: (exercise, bucket)
# ──────────────────────────────────────────────────────────────
class _RecentPicker:
    def __init__(self):
        # last_chosen[("squat","danger")] = "문구..."
        self.last_chosen: Dict[tuple, str] = {}
        self.rng = random.Random()

    def set_seed(self, seed: Optional[int]):
        if seed is not None:
            self.rng.seed(seed)

    def choose(self, exercise: str, bucket: str, candidates: List[str]) -> str:
        if not candidates:
            return ""

        key = (exercise, bucket)
        last = self.last_chosen.get(key)

        # 후보가 1개면 그대로 반환
        if len(candidates) == 1:
            chosen = candidates[0]
            self.last_chosen[key] = chosen
            return chosen

        # 직전 문구 제외한 후보 생성
        pool = [c for c in candidates if c != last] if last in candidates else candidates[:]

        # 만약 모두 제외되어 빈 리스트가 됐다면(동일 문구 1개만 있는 특수 상황)
        if not pool:
            pool = candidates[:]

        chosen = self.rng.choice(pool)
        self.last_chosen[key] = chosen
        return chosen


_picker = _RecentPicker()


def set_random_seed(seed: Optional[int]) -> None:
    """테스트/재현 목적의 시드 고정(선택사항)."""
    _picker.set_seed(seed)


# ──────────────────────────────────────────────────────────────
# 내부: 기본 멘트 1개 선택(+즉시 반복 방지)
# ──────────────────────────────────────────────────────────────
def _pick_base(exercise: str, bucket: str) -> str:
    ex = exercise if exercise in BASE else "generic"
    arr = BASE.get(ex, {}).get(bucket) or BASE["generic"][bucket]
    return _picker.choose(ex, bucket, arr)


# ──────────────────────────────────────────────────────────────
# 공개 API: 최종 코칭 멘트 생성
# ──────────────────────────────────────────────────────────────
def get_advice(exercise: str, score: Optional[int], ctx: Optional[Dict[str, bool]] = None) -> str:
    """
    exercise: "squat"|"leg_raise"|"shoulder_press"|"lateral_raise"|"pushup"|...
    score   : 0~100
    ctx     : {"elbow_not_deep": True, "knee_more_extend": True, ...}
    """
    bucket = score_bucket(score)
    base = _pick_base(exercise, bucket)

    if not ctx:
        return base

    tips = [TIP_MAP[k] for k, v in ctx.items() if v and k in TIP_MAP]
    if not tips:
        return base

    # 기본 멘트 + 보정 팁 결합
    return f"{base} " + " ".join(tips)



    ### 사운드 재생 ### 

    def get_advice_with_sfx(
        exercise: str,
        score: Optional[int],
        ctx: Optional[Dict[str, bool]] = None
        ) -> tuple[str, str, str]:
        
        bucket = score_bucket(score)
        text = get_advice(exercise, score, ctx)
        exercise_norm = (exercise or "generic").strip().lower().replace("-", "_")
        sfx_key = f"advice_{exercise_norm}_{bucket}"
        return text, bucket, sfx_key

